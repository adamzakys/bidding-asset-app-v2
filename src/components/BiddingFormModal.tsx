...
export const BiddingFormModal: React.FC<BiddingFormModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md bg-[#f1f2f5] dark:bg-[#1e293b] border border-white dark:border-slate-800 shadow-2xl rounded-t-3xl sm:rounded-3xl p-6 sm:p-8 relative animate-fade-in-up max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Pull handle bar for mobile bottom-sheet feel */}
        <div className="w-12 h-1.5 bg-slate-300 dark:bg-slate-700 rounded-full mx-auto mb-4 sm:hidden cursor-pointer" onClick={onClose} />

        <button
          onClick={onClose}
          className="btn-neu absolute top-4 right-4 h-8 w-8 flex items-center justify-center rounded-full text-slate-500 hover:text-rose-500 transition cursor-pointer text-xs font-black hidden sm:flex border border-white/60 dark:border-slate-800"
          title="Tutup Modal"
        >
          ✕
        </button>

        <div className="flex justify-between items-center pb-3 border-b border-slate-200/40 dark:border-slate-800/40 mb-4">
          ...
          <button
            onClick={onClose}
            className="btn-neu px-3 py-1.5 text-[10px] font-bold text-slate-600 dark:text-slate-350 hover:text-rose-500 cursor-pointer sm:hidden"
          >
            Tutup
          </button>
        </div>
